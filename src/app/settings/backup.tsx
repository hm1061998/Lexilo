import { useBackupHistory, useCreateBackup } from '@/features/backup/hooks/use-backup';
...
export default function BackupScreen() {
  ...
  return (
    ...
    >
      ...
      <AppButton
        label={create.isPending ? 'Đang tạo…' : 'Tạo và chia sẻ bản sao lưu'}
        disabled={create.isPending}
        onPress={() =>
          create.mutate(undefined, {
            onSuccess: async (r) => {
              if (await Sharing.isAvailableAsync())
                await Sharing.shareAsync(r.uri, { mimeType: 'application/json' });
            },
          })
        }
      />
      {history.data?.map((x) => (
        <View key={x.id}>
          <Text style={{ color: colors.text }}>{x.file_name ?? 'Backup'}</Text>
          <Text style={{ color: colors.textMuted }}>
